...
/*----------------------------- Module Defines ----------------------------*/
#define RECEIVE_PORT _Port_A
#define RECEIVE_PIN _Pin_2
#define RECEIVE_VAL PORTAbits.RA2
#define TRANSMIT_PORT _Port_A
#define TRANSMIT_PIN _Pin_3
#define TRANSMIT_VAL LATAbits.LATA3
...
static uint8_t ValidReceiveLengths[] = {15, 35};
const uint8_t SZ_VALIDRECEIVE = sizeof(ValidReceiveLengths)/sizeof(ValidReceiveLengths[0]);
const ES_EventType_t ReceiveEvents[] = {ES_START, ES_RESET};
static const uint8_t RECEIVE_ERROR = 5;

static bool LastReceiveState;
static uint16_t ReceiveRisingTime;

uint8_t EventType2Length(ES_EventType_t event)
{
    if(event == ES_SUCCESS)
    {
        return ValidReceiveLengths[0];
    }else if(event == ES_FAILURE)
    {
        return ValidReceiveLengths[1];
    }
}
/*------------------------------ Module Code ------------------------------*/
/****************************************************************************
...
//    PortSetup_ConfigurePullDowns(RECEIVE_PORT, RECEIVE_PIN);
  ...
  switch (CurrentState)
  {
    case P2P_TransmitIdle:        // If current state is idle
    {
        if (ThisEvent.EventType == P2P_START_TRANSMIT)
        {
            ...
            TransmitLength = EventType2Length(ThisEvent.EventParam);
            CurrentState = P2P_SendingLo;
            ES_Timer_InitTimer(P2PTransmit_TIMER, TransmitLength);
            DB_printf("P2PComm: Starting Transmit with length = %u\r\n", TransmitLength);
        }
    }
    break;
        ...
        else // falling edge
        {
            ...
            for (idx = 0; idx < SZ_VALIDRECEIVE; idx++)
            {
                       ...
                       (HiTime <= (ValidReceiveLengths[idx] + RECEIVE_ERROR)) )
                {
                    ...
                    PostEvent.EventType = ReceiveEvents[idx];
                    PostPIC2ButtonFSM(PostEvent);
//                    ES_PostList00(PostEvent); // post to distro 
