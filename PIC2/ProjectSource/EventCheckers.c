...
{
    static uint16_t debounceTimer[NUM_BTNS] = {0,0,0,0};
    static bool lastPinState[NUM_BTNS] = {0,0,0,0};
    static debounce_state_t bounceStates[NUM_BTNS] = {Waiting,Waiting,Waiting,Waiting};
    const uint16_t debounceLim = 10;
    static bool inited = 0;
    bool returnval = 0;
    if(!inited){
        lastPinState[0] = VODKA_BTN;
        lastPinState[1] = GIN_BTN;
        lastPinState[2] = ICE_BTN;
        lastPinState[3] = OLIVE_BTN;
        inited = 1;
        return false;
    }
//    ES_Event_t newEvent;
//    newEvent.EventType = ES_RISE;
//    PostPIC2ButtonFSM(newEvent);
    bool curPinState[NUM_BTNS] = {VODKA_BTN,GIN_BTN,ICE_BTN,OLIVE_BTN};
    uint16_t cur_Time = ES_Timer_GetTime();
//    sendToDisPlay(bounceStates);
    for(uint8_t i = 0; i < NUM_BTNS; i++){
        if(curPinState[i] != lastPinState[i]){
            if(curPinState[i] == 0 && bounceStates[i] == Waiting){
                debounceTimer[i] = cur_Time;
                bounceStates[i] = DebounceFall;
            }else if(curPinState[i] == 1 && bounceStates[i] == Waiting){
                debounceTimer[i] = cur_Time;
                bounceStates[i] = DebounceRise;                
            }else if(curPinState[i] == 0 && bounceStates[i] == DebounceRise){
                bounceStates[i] = Waiting;
            }else if(curPinState[i] == 1 && bounceStates[i] == DebounceFall){
                bounceStates[i] = Waiting;
            }else{
                DB_printf("Something's Wrong\r\n");
            }
            lastPinState[i] = curPinState[i];
//            sendToDisPlay(curPinState);
        }else{
            if(bounceStates[i] != Waiting && ((cur_Time - debounceTimer[i]) > debounceLim)){
//                DB_printf("change detected%d\r\n",i);
                ES_Event_t newEvent;
                newEvent.EventType = bounceStates[i] == DebounceRise ? ES_RISE:ES_FALL;
//                DB_printf("rise")
                newEvent.EventParam = i;
                bounceStates[i] = Waiting;
                PostPIC2ButtonFSM(newEvent);
                returnval = 1;
//                sendToDisPlay(curPinState);
