...
{
    uint8_t WhichRow;
    for (WhichRow = 3; WhichRow < NUM_ROWS; WhichRow++)
    {
        DM_Display[WhichRow].ByBytes[0]  |= getFontLine(Char2Display, WhichRow-3);
    }
}

void DM_AddFilledSection(void)
{
    uint8_t WhichRow;
    for (WhichRow = 0; WhichRow < NUM_ROWS; WhichRow++)
    {
        DM_Display[WhichRow].ByBytes[0]  |= 0xFF & (~(1<<7));
    } 
}

void DM_AddFirstCol(void)
{
    uint8_t WhichRow;
    for (WhichRow = 0; WhichRow < NUM_ROWS; WhichRow++)
    {
        DM_Display[WhichRow].ByBytes[0]  |= 0x01;
    } 
}
